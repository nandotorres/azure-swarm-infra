...
azure availset create -n "manager-availset" -g "docker-swarm" -l "eastus"
...
for vm in $vms; do
  ...
  if [[ $vm =~ manager[1-3] ]]; then
    azure vm create docker-swarm $vm eastus --nic-name "$vm-nic" -y "Linux" -Q "UbuntuLTS" -u "nandotorres" --ssh-publickey-file ~/.ssh/id_rsa.pub -z "Basic_A1" --vnet-name "swarm-vnet" --vnet-subnet-name "default" --public-ip-name "$vm-IP" --custom-data "./provision.sh" --availset-name "manager-availset"
  else
