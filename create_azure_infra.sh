...
azure network nsg rule create -g "docker-swarm" -a "swarm-nsg" -n "HTTP_8080" -d "Allow HTTP Variation" -p "TCP" -f "0.0.0.0/24" -o "8080" -e "0.0.0.0/24" -c "Allow" -r "Inbound" -y "110"

vms="manager1 manager2 manager3 worker1 worker2 worker3"

for vm in $vms; do
  azure network public-ip create docker-swarm "$vm-IP" eastus
  azure network nic create -n "$vm-nic" -l eastus -o "swarm-nsg" -g "docker-swarm"
